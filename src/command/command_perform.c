...
void validate_cursor_position(context *context) {
  ...
  if ((*context).cursor.position_x > max_x + 1) {
    (*context).cursor.position_x = max_x + 1;
  }
}
...
void command_perform(command command, context *context) // PUBLIC;
{
  ...
  case INSERT: {
    ...
    mutable_string *mutable_string =
        mutable_string_select_position_x(head->mutable_string, context->cursor.position_x, &byte);
  ...
  case DELETE: {
    if (context->cursor.position_x > 1) {
      ...
      lines *head = lines_select_position_y(context->lines, context->cursor.position_y);
      mutable_string *mutable_string =
          mutable_string_select_position_x(head->mutable_string, context->cursor.position_x - 1, &byte);
      ...
      (*context).cursor.position_x =
          lines_select_position_y((*context).lines, (*context).cursor.position_y - 1)->position_count;
      (*context).cursor.position_y -= 1;
    }
  } break;
  case ENTER: {
